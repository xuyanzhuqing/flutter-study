...
class TestFlutterMap extends StatelessWidget {
  @override
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text((controller.zoom).toString()),
        actions: [
                ...
                children: [
                  Map(
                    ...
                    builder: (context, x, y, z) {
                      ...
                      String access_token = 'pk.eyJ1IjoibWljaGFlbGphY2siLCJhIjoiY2o2bXlxNG9kMDM4bjM0cG15dmU4NWVweSJ9.M9Z40pnx2VYWrQ6GNd4o_g';
                      // int zoom = getZoom(controller.zoom);
                      int zoom = controller.zoom.round();
                      final url = 'https://api.mapbox.com/v4/mapbox.satellite/${zoom}/$x/$y@2x.jpg90?access_token=$access_token';
  ...
  // 根据传入的zoom, 计算最接近的zoom
  int getZoom (zoom) {
    List<int> zoomLevel = [0, 3, 4, 6, 10, 15, 22];
    List<num> tmp = zoomLevel.map((v) {
      return (v - zoom).abs();
    }).toList();
